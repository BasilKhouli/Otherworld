...
public class Howl extends Ability {
    ...
    private void preformHowl(float deltaTime, Ref<EntityStore> ref, PlayerRef playerRef, Store<EntityStore> store, CommandBuffer<EntityStore> commandBuffer, Vector3d position) {
        ...
        for (Ref<EntityStore> entityRef : results){
            ...
            applyKnockback(commandBuffer, position, entityRef, entityPos);
    ...
    private static void applyKnockback(CommandBuffer<EntityStore> commandBuffer, Vector3d position, Ref<EntityStore> entityRef, Vector3d entityPos) {
